    ...
    <div class="w-full bg-[#eeeeee] py-1 md:px-15 border-t border-gray-100">
        <div class="relative group">
            ...
            <Swiper :modules="modules" :slides-per-view="1" :space-between="6" :loop="true" :navigation="true"
                ...
                }" class="swiper-ambiences">

                <SwiperSlide v-for="ambience in ambiences" :key="ambience.slug">

                    <div class="relative w-full aspect-square md:aspect-[4/5] overflow-hidden">
                        ...
                        <div v-for="(hotspot, idx) in ambience.hotspots" :key="idx"
                            class="absolute z-20 w-8 h-8 flex items-center justify-center cursor-pointer group/hotspot transform -translate-x-1/2 -translate-y-1/2"
                            ...
                            <Link :href="`/products/${hotspot.product_slug}`"
                                class="absolute bottom-full mb-2 bg-white text-gray-800 text-[10px] font-bold uppercase tracking-widest z-[99] py-2 px-4 shadow-lg opacity-0 group-hover/hotspot:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none group-hover/hotspot:pointer-events-auto hidden md:block">
